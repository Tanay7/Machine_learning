...
# Console Handler (so you can see progress in the terminal)
...
def main():
    ...
    try:
        # 1. Fetch Data
        # --------------------------------------------------
        select_query = f"SELECT id, name_only, gender, dob, address, occupation, bank_acct_no FROM {schema}.{INPUT_TABLE}"
        ...
        # Convert to dictionary format required by Dedupe
        # format: {id: {col1: val1, col2: val2}, ...}
        data_d = {}
        for row in data:
            row_dict = dict(zip(columns, row))
            row_id = row_dict['id']
            # Ensure None values are handled if necessary, usually Dedupe handles None gracefully
            data_d[row_id] = row_dict

        logger.info(f'{len(data_d)} records imported.')
        
        # Memory Cleanup: Delete raw list to free RAM
        del data 
        ...
        if os.path.exists(SETTINGS_FILE):
            ...
            # CRITICAL FIX: StaticDedupe cannot run without a settings file.
            # If you need to train a new model, you must use dedupe.Dedupe() and active labeling,
            # which is interactive and cannot run in this automated script flow.
            logger.error(f"Settings file '{SETTINGS_FILE}' not found. Cannot proceed with static deduplication.")
            sys.exit(1)
        ...
        for cluster_id, (cluster_members, scores) in enumerate(clustered_dupes):
            # deduper.partition returns a generator of (cluster_ids, scores)
            # We assign an integer ID to the cluster based on enumeration
            
            for cust_id, score in zip(cluster_members, scores):
                # Ensure types are native Python types for psycopg2
        ...
        if count_result == total_inserted:
            ...
            logger.warning(f'Row count mismatch! Inserted: {total_inserted}, Found in DB: {count_result}')
            connection.commit() # Commit anyway, but warn user
